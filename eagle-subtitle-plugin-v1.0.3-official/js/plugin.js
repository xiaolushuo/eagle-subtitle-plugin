/**
 * Eagle Subtitle Plugin - Main Plugin Script
 * This is the main plugin script that handles subtitle functionality
 */
...
async function initialize() {
    ...
    try {
        ...
        updateStatus('插件初始化成功');
        
    } catch (error) {
        console.error('❌ Failed to initialize plugin:', error);
        updateStatus('插件初始化失败: ' + error.message);
    }
}
...
async function handleFileSelection() {
    try {
        ...
        updateStatus('处理文件选择时出错: ' + error.message);
    }
}
...
async function loadSubtitlesForVideo(videoItem) {
    try {
        ...
        updateStatus(`已加载 ${pluginState.currentSubtitles.length} 条字幕`);

    } catch (error) {
        console.error('❌ Error loading subtitles:', error);
        updateStatus('加载字幕时出错: ' + error.message);
    }
}
...
async function createSubtitleOverlay() {
    try {
        ...
        updateStatus('字幕覆盖层已创建');

    } catch (error) {
        console.error('❌ Error creating overlay:', error);
        updateStatus('创建字幕覆盖层时出错: ' + error.message);
    }
}
...
async function adjustTimeOffset() {
    try {
        ...
        if (offset !== null) {
            ...
            updateStatus(`时间偏移已设置为: ${pluginState.timeOffset}秒`);
        }
    } catch (error) {
...
function testPlugin() {
    ...
    updateStatus('正在测试插件功能...');
    
    // Test basic functionality
    setTimeout(() => {
        if (pluginState.initialized) {
            updateStatus('插件测试通过 - 已初始化');
        } else {
            updateStatus('插件测试失败 - 未初始化');
        }
    }, 1000);
}
...
async function reloadSubtitles() {
    console.log('🔄 Reloading subtitles...');
    updateStatus('正在重新加载字幕...');
    
    if (pluginState.currentVideo) {
        await loadSubtitlesForVideo(pluginState.currentVideo);
    } else {
        updateStatus('没有当前视频文件');
    }
}
...
// Initialize plugin when Eagle is ready
if (typeof eagle !== 'undefined') {
    ...
    // For testing outside of Eagle
    console.log('⚠️ Eagle not available, running in test mode');
    document.addEventListener('DOMContentLoaded', () => {
        updateStatus('Eagle 环境未检测到，运行在测试模式');
    });
}
